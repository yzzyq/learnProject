...
var seckill = {
    ...
    URL : {
        ...
        result:function (seckill_id){
            return '/seckill/'+ seckill_id + '/getSeckillResult';
        },
                    ...
                    $('#killBtn').one('click',function (){
                        ...
                        $.post(killUrl,{},function (result){
                            if(result && result['success']){
                                ...
                                var kill_result = seckill.URL.result(seckill_id);



                                seckill.sleep(500).then(() => {
                                    if(state == 2) {
                                        console.log(state);
                                        $.post(kill_result, {}, function (result) {
                                            killResult = result['data'];
                                            state = killResult['state'];
                                            stateInfo = killResult['statInfo'];
                                            node.html('<span class="label label-success">' + stateInfo + '</span>');
                                        });
                                    }
                                });
